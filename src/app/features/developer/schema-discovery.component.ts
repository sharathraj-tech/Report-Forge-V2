...
@Component({
  ...
  template: `
    <div class="flex flex-col bg-white anim-fade">
      <div class="px-5 py-4 border-b flex flex-col gap-4 bg-slate-50/50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-[11px] font-black text-slate-900 tracking-widest leading-tight uppercase">Catalog Explorer</h2>
            <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Schema Onboarding</p>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3 pt-3 border-t border-slate-100">
           <div class="flex items-center justify-between">
              <div class="flex items-center gap-1 -ml-1">
                 <button class="text-[9px] font-black uppercase text-brand-primary hover:bg-slate-100 px-2 py-1 rounded transition-colors" (click)="selectAll()" [disabled]="!tables().length">Select All</button>
                 <span class="text-slate-300">•</span>
                 <button class="text-[9px] font-black uppercase text-slate-500 hover:bg-slate-100 px-2 py-1 rounded transition-colors" (click)="selectedTables.set([])" [disabled]="!selectedTables().length">Clear</button>
              </div>
              <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest"><span class="text-brand-primary">{{ selectedTables().length }}</span> / {{ tables().length }} Objects</span>
           </div>
           
           <div class="flex items-center gap-2">
             <button 
                class="rf-compact-btn-primary h-8 flex-1 justify-center shadow-sm"
      ...
      <div class="p-2 h-[450px] overflow-hidden flex flex-col bg-white">
          <dx-list
             [items]="tables()"
             [searchEnabled]="true"
             searchExpr="this"
             ...
             <div *dxTemplate="let item of 'item'" class="flex items-center justify-between p-2 my-0.5 group rounded-lg transition-all border border-transparent hover:border-slate-200 hover:bg-slate-50">
                <div class="flex items-center gap-2.5">
                  <div class="w-7 h-7 rounded-lg bg-slate-100 flex items-center justify-center group-hover:bg-white transition-colors">
                    <span class="material-icons text-slate-400 text-sm group-hover:text-brand-primary">{{ item.includes('.') ? 'table_view' : 'fact_check' }}</span>
                  </div>
                ...
                <button class="h-6 px-3 rounded-md text-[8px] font-black tracking-widest transition-all" 
                   [class]="isImported(item) ? 'bg-emerald-50 text-emerald-600' : 'bg-white text-brand-primary border border-slate-200 hover:bg-brand-primary hover:text-white'"
  ...
  styles: [`
    :host { display: block; }
    .compact-discovery-list { background: transparent !important; border: none !important; }
    ...
    ::ng-deep .compact-select .dx-texteditor-input { padding: 8px 12px !important; font-size: 11px !important; font-weight: 700 !important; }
    ::ng-deep .compact-select { border-radius: 8px !important; background: white !important; border: 1px solid #e2e8f0 !important; }
